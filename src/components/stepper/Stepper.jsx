      ...
      if (response?.data?.features[0]?.geometry?.coordinates[0]) {
        ...
        async function makeAPICall() {
          ...
          while (!isValidJson) {
            ...
            try {
              console.log('try')
              gptResponse = await axios.post("http://localhost:8080/trip-details", {
                data: parmas,
              });
        
              if (
                (gptResponse.status === 200 || gptResponse.status === 201) &&
                gptResponse.data
              ) {
                let parsedObj = gptResponse.data?.response;
                var match = parsedObj.match(jsonPattern);
                let match2 = match[0]?.match(jsonPattern);
        
                if (typeof JSON.parse(match2) === "object") {
                  isValidJson = true;
